...
Source: [wolf99/dotfiles/gpg-setup.md](https://github.com/wolf99/dotfiles/blob/master/gpg-setup.md)
...
4. Then add ultimate trust for it by typing `trust` at the prompt and selecting
   ultimate trust.
...
1. If there are no existing GPG key pairs or none suitable, generate a new one
   (t least 4096 bits, user ID in form `<id>+<username>@users.noreply.github.com`)
...
3. Configure Git to use the key for every commit (this will apply even to repos
   that are not GitHub ones :| )
...
- May or may not need `echo 'export GPG_TTY=$(tty)' >> ~/.bashrc` for the
  gpg-agent... not sure.
